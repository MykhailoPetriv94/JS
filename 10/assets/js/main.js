...
$(function() {
    ...
    const background = $('<div></div>');
          background.addClass('container__content-background');
          background.appendTo(containerContent);        

    const backgroundCov = $('<div></div>');
          backgroundCov.addClass('container__content-backgroundCov');
          backgroundCov.appendTo(containerContent);
        
    const main = $('<div></div>');
          main.addClass('game__info');
          main.appendTo(containerContent);

    const options = $('<div></div>');
          options.addClass('options');
          options.appendTo(main);

    //NAVIGATION/ITEMS//
       
    let itemInfo = $('<div></div>');
        itemInfo.addClass('item');
        itemInfo.appendTo(options);
        itemInfo.attr('id', 'information');
                    
    const infoLogo = $('<div></div>');
          infoLogo.addClass('logo');
          infoLogo.appendTo(itemInfo);

    const imgFirst = $('<svg width="26" height="24" viewbox="0 0 26 24" fill="none"> <path class="image__element" d="M6.33333 6.26304H19.6667H6.33333ZM6.33333 10.2103H19.6667H6.33333ZM6.33333 14.1576H11.6667H6.33333ZM16.7707 19.4206H22.3333C23.0406 19.4206 23.7189 19.1434 24.219 18.6499C24.719 18.1564 25 17.487 25 16.7891V3.63152C25 2.9336 24.719 2.26426 24.219 1.77075C23.7189 1.27725 23.0406 1 22.3333 1H3.66667C2.95942 1 2.28115 1.27725 1.78105 1.77075C1.28095 2.26426 1 2.9336 1 3.63152V16.7891C1 17.487 1.28095 18.1564 1.78105 18.6499C2.28115 19.1434 2.95942 19.4206 3.66667 19.4206H7.91733C9.25067 19.4206 10.3333 20.4877 10.3333 21.8048C10.3333 22.8679 11.6347 23.3995 12.396 22.6482L14.8853 20.1917C15.3853 19.6981 16.0635 19.4208 16.7707 19.4206V19.4206Z" stroke="#8D91A3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>');
          imgFirst.addClass('image');
          imgFirst.appendTo(infoLogo);
                    
    const infoText = $('<p></p>');
          infoText.addClass('text');
          infoText.text('INFORMATION');
          infoText.appendTo(itemInfo);

    //________//      

    let itemRating = $('<div></div>');
        itemRating.addClass('item');
        itemRating.appendTo(options);
        itemInfo.attr('id', 'rating');
 
    const ratingLogo = $('<div></div>');
          ratingLogo.addClass('logo');
          ratingLogo.appendTo(itemRating);

    const imgSecond = $('<svg width="22" height="24" viewbox="0 0 22 24" fill="none"><path class="image__element" d="M11 14.4459C14.3333 19.2865 21 19.2542 21 13.9081C21 9.6054 16.5556 6.37837 11 1C5.44444 6.37837 1 9.6054 1 13.9081C1 19.2542 7.66667 19.2865 11 14.4459Z" stroke="#8D91A3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path class="image__element"  d="M10.4112 15.3736L7.07821 21.9663C7.02411 22.0735 6.99724 22.1936 7.00022 22.315C7.0032 22.4364 7.03593 22.5549 7.09522 22.659C7.15451 22.7631 7.23834 22.8492 7.33855 22.909C7.43876 22.9687 7.55193 23.0001 7.66704 23H14.333C14.4481 23.0001 14.5612 22.9687 14.6615 22.909C14.7617 22.8492 14.8455 22.7631 14.9048 22.659C14.9641 22.5549 14.9968 22.4364 14.9998 22.315C15.0028 22.1936 14.9759 22.0735 14.9218 21.9663L11.5888 15.3736C11.5321 15.2608 11.4473 15.1664 11.3436 15.1006C11.2398 15.0348 11.1211 15 11 15C10.8789 15 10.7602 15.0348 10.6564 15.1006C10.5527 15.1664 10.4679 15.2608 10.4112 15.3736V15.3736Z" stroke="#8D91A3" stroke-width="2" stroke-linecap="round"/></svg>');
          imgSecond.addClass('image');
          imgSecond.appendTo(ratingLogo);
                    
    const ratingText = $('<p></p>');
          ratingText.addClass('text');
          ratingText.text('RATINGS INFO');
          ratingText.appendTo(itemRating);

    //________//      

    let itemPurchase = $('<div></div>');
        itemPurchase.addClass('item');
        itemPurchase.appendTo(options);
        itemInfo.attr('id', 'purchase');
                    
    const purchaseLogo = $('<div></div>');
          purchaseLogo.addClass('logo'); 
          purchaseLogo.appendTo(itemPurchase);

    const imgThird = $('<svg width="24" height="22" viewbox="0 0 24 22" fill="none"><path class="image__element--fill" d="M23.9899 20.0403L22.4453 5.94854C22.3998 5.51745 22.199 5.11889 21.8817 4.83008C21.5644 4.54127 21.1532 4.38275 20.7278 4.38524H17.1092C16.9013 3.1578 16.2746 2.04469 15.3398 1.24234C14.405 0.439998 13.2221 0 12 0C10.7779 0 9.59503 0.439998 8.66022 1.24234C7.72542 2.04469 7.09873 3.1578 6.89078 4.38524H3.2722C2.8468 4.38275 2.43563 4.54127 2.11833 4.83008C1.80104 5.11889 1.6002 5.51745 1.55472 5.94854L0.0100746 20.0403C-0.0160867 20.2878 0.00884007 20.538 0.0832522 20.775C0.157664 21.012 0.279915 21.2305 0.442144 21.4165C0.603832 21.6007 0.801973 21.748 1.02353 21.8486C1.24508 21.9491 1.48502 22.0007 1.72755 22H22.2724C22.515 22.0007 22.7549 21.9491 22.9765 21.8486C23.198 21.748 23.3962 21.6007 23.5579 21.4165C23.7201 21.2305 23.8423 21.012 23.9167 20.775C23.9912 20.538 24.0161 20.2878 23.9899 20.0403ZM12 1.74302C12.7661 1.74451 13.5102 2.00457 14.116 2.48259C14.7218 2.96061 15.1552 3.62965 15.3485 4.38524H8.65146C8.84478 3.62965 9.27819 2.96061 9.88399 2.48259C10.4898 2.00457 11.2339 1.74451 12 1.74302ZM1.72755 20.2385L3.2722 6.14671H6.81517V8.78892C6.81517 9.02251 6.90621 9.24653 7.06827 9.4117C7.23033 9.57687 7.45012 9.66966 7.67931 9.66966C7.90849 9.66966 8.12829 9.57687 8.29034 9.4117C8.4524 9.24653 8.54345 9.02251 8.54345 8.78892V6.14671H15.4566V8.78892C15.4566 9.02251 15.5476 9.24653 15.7097 9.4117C15.8717 9.57687 16.0915 9.66966 16.3207 9.66966C16.5499 9.66966 16.7697 9.57687 16.9317 9.4117C17.0938 9.24653 17.1848 9.02251 17.1848 8.78892V6.14671H20.7278L22.2724 20.2385H1.72755Z" fill="#8D91A3" /></svg>');
          imgThird.addClass('image');
          imgThird.appendTo(purchaseLogo);
                    
    const purchaseText = $('<p></p>');
          purchaseText.addClass('text');
          purchaseText.text('WHERE TO BUY');
          purchaseText.appendTo(itemPurchase);

    // ITEM ONCLICK//

    $('.options').on('click', '.optionsItem:not(.active)', function(event) {
      console.log('PRESSED OPTION')
      $(this).addClass('active').siblings().removeClass('active')
      .closest('div.game__info').find('div.card').removeClass('active').eq($(this).index()).addClass('active');
  })    
    ...
    $('.rating').on('click', function(event) {
      ...
      $('.circle__fill').removeClass('notShow').addClass('display');
      $('.circle__value').removeClass('notShow').addClass('display');
      return
      });    
    ...
    const cardRight = $('<div></div>');
          cardRight.addClass('text__info');
          cardRight.appendTo(cardContainer);

    //DESCRIPTION//      
    const description = $(`<div></div>`);
          description.addClass('description');
          description.appendTo(cardRight);
    ...
    const descriptionNote = $('<span></span>');
          descriptionNote.text('First Released Oct 6, 2021');
          descriptionNote.addClass('description__note');
          descriptionNote.appendTo(description);
    ...
    const descriptionText = $('<span></span>');
          ...
          descriptionText.appendTo(description) ;    
                            
    //CIRCLE RATING//
    const circle = $('<div></div>');
          circle.addClass('circle');
          circle.appendTo(description);

    const circleEmpty = $('<svg width="73" height="73" viewbox="0 0 73 73" fill="none"><path d="M73 36.5C73 56.6584 56.6584 73 36.5 73C16.3416 73 0 56.6584 0 36.5C0 16.3416 16.3416 0 36.5 0C56.6584 0 73 16.3416 73 36.5ZM1.825 36.5C1.825 55.6505 17.3495 71.175 36.5 71.175C55.6505 71.175 71.175 55.6505 71.175 36.5C71.175 17.3495 55.6505 1.825 36.5 1.825C17.3495 1.825 1.825 17.3495 1.825 36.5Z" fill="#B7BACD"/></svg>')
          circleEmpty.addClass('circle__empty');
          circleEmpty.appendTo(circle);
                                    
    const circleFill = $('<svg width="75" height="75" viewbox="0 0 75 75" fill="none"><path d="M10.5248 11.4503C4.53315 17.6548 0.863513 25.7351 0.13427 34.3295C-0.594973 42.9239 1.66076 51.5069 6.52133 58.6322C11.3819 65.7576 18.5501 70.9895 26.8181 73.4464C35.086 75.9034 43.9482 75.435 51.9111 72.1204C59.8741 68.8057 66.4509 62.8474 70.5334 55.2494C74.6159 47.6515 75.9545 38.8785 74.3234 30.4088C72.6924 21.9392 68.1916 14.2907 61.5794 8.75225C54.9672 3.21382 46.6479 0.124056 38.0235 0.00365383L37.9711 3.75329C45.7331 3.86165 53.2205 6.64243 59.1714 11.627C65.1224 16.6116 69.1732 23.4953 70.6411 31.118C72.109 38.7407 70.9043 46.6364 67.2301 53.4745C63.5558 60.3126 57.6367 65.6751 50.47 68.6583C43.3034 71.6415 35.3274 72.063 27.8863 69.8518C20.4451 67.6406 13.9937 62.9318 9.61919 56.519C5.24468 50.1062 3.21452 42.3815 3.87084 34.6465C4.52716 26.9116 7.82983 19.6393 13.2223 14.0553L10.5248 11.4503Z" fill="url(#paint0_linear_1_71)"/><defs><linearGradient id="paint0_linear_1_71" x1="26.5" y1="-7.5" x2="5.5" y2="64" gradientUnits="userSpaceOnUse"><stop stop-color="#00D4BE"/><stop offset="0.1875" stop-color="#1AE780"/><stop offset="0.833333" stop-color="#1AE780"/><stop offset="1" stop-color="#00D4BE"/></linearGradient></defs></svg>')
          circleFill.addClass('circle__fill notShow');
          circleFill.appendTo(circle);
                           
    const circleValue = $('<span></span>');
          circleValue.text('8.7');
          circleValue.addClass('circle__value notShow');
          circleValue.appendTo(circle);


    //PURCHASE//      
    const purchase = $('<div></div>');
          purchase.addClass('purchase');
          purchase.appendTo(cardRight);

    const purchasePrice = $('<div></div>');
          purchasePrice.addClass('purchase__price');
          purchasePrice.appendTo(purchase);

    const discount = $('<p></p>').text('20% OFF!').addClass('discount').appendTo(purchasePrice);
                                
    const price = $('<p></p>').text('$59.96').addClass('price').appendTo(purchasePrice);   

    const button = $('<button type="submit"></button>').text('BUY IT NOW').addClass('button').appendTo(purchase);
})